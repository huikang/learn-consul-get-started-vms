...
license_path = "/tmp/consul-license"
EOF

  # bootstrap_expect only for the first consul server
  if [ $i == "0" ]; then
    echo "bootstrap_expect = 1" >>./agent-server-specific.hcl
  fi

  # get server publish ip address
  publicIPaddr=$(ssh -i ~/certs/id_rsa consul-server-$i timeout 1 ec2metadata --public-ipv4)
